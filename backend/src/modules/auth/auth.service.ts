...
export class AuthService {
  ...
  async login(dto: LoginDto, ipAddress: string): Promise<AuthTokensDto> {
    ...
    if (!valid) {
      ...
      if (attempts >= MAX_LOGIN_ATTEMPTS) {
        await this.redis.set(lockKey, '1', 'EX', LOCK_TTL_SECONDS).catch(() => null);
      }
  ...
  async refreshToken(rawToken: string): Promise<AuthTokensDto> {
    ...
    for (const r of records) {
      if (!r.isRevoked && r.expiresAt > new Date() && await bcrypt.compare(rawToken, r.tokenHash)) {
        found = r;
        break;
      }
    }
  ...
  private async issueTokens(user: UserEntity): Promise<AuthTokensDto> {
    ...
    const accessToken = this.jwtService.sign(payload, {
      privateKey: this.configService.get('app.jwtPrivateKey'),
      algorithm: 'RS256',
      expiresIn: '15m',
    });
